...
const ForgotPassword = ({navigation}) => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            const { data} = await axios.post("http://172.19.115.28:8000/api/forgot-password", {email})
    ...
    const handlePasswordReset = async () => {
        try {
            const { data  } = await axios.post('http://172.19.115.28:8000/api/reset-password', { email, resetCode, password})
            if (data.error) alert(data.error)
            else {
